...
def simple_arima_forecast(train_data, steps=1):
    """Simple ARIMA implementation without pmdarima"""
    try:
        # Use simple ARIMA parameters (1,1,1) as default
        ...
        return np.full(steps, train_data.rolling(window=3).mean().iloc[-1]), None
...
def task2_forecasting_models(monthly_total_ts):
    ...
    # ARIMA Model
    try:
        arima_forecast, arima_fit = simple_arima_forecast(train_data['Volume'], len(test_data))
    ...
    if SKLEARN_AVAILABLE:
        try:
            ...
            best_model = 'ARIMA'
            st.info("Using ARIMA model (evaluation metrics unavailable)")
    else:
        best_model = 'ARIMA'
...
def task3_forecast_future(monthly_total_ts, model_results, forecast_months=12):
    ...
    if best_model_type == 'ARIMA' and STATSMODELS_AVAILABLE:
        try:
            # Use the trained ARIMA model if available
            if model_results['arima_model'] is not None:
                arima_model = model_results['arima_model']
                future_forecast = arima_model.forecast(steps=forecast_months)
            else:
                # Fallback: train a new simple ARIMA model
                future_forecast, _ = simple_arima_forecast(monthly_total_ts, forecast_months)
            ...
            st.warning(f"ARIMA future forecast failed: {e}")
...
def main():
    ...
    if uploaded_file is not None:
        ...
        with col1:
            ...
            st.markdown("""
            - **Sales Forecasting**: Predict future sales using ARIMA models
